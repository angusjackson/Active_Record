	...
	public sealed class SQLiteRoleProvider : RoleProvider
	{
			...
			try
			{
				...
				using (SQLiteCommand cmd = cn.CreateCommand())
				{
					cmd.CommandText = "INSERT INTO " + USERS_IN_ROLES_TB_NAME
						...
						+ " AND (r.RoleName = $RoleName) AND (r.ApplicationId = $ApplicationId)";
			...
			try
			{
				using (SQLiteCommand cmd = cn.CreateCommand())
				{
					cmd.CommandText = "INSERT INTO " + ROLE_TB_NAME
						+ " (RoleId, RoleName, ApplicationId) "
						+ " Values ($RoleId, $RoleName, $ApplicationId)";
		...
		public override bool DeleteRole(string roleName, bool throwOnPopulatedRole)
		{
			...
			try
			{
				...
				using (SQLiteCommand cmd = cn.CreateCommand())
				{
					cmd.CommandText = "DELETE FROM " + ROLE_TB_NAME + " WHERE RoleName = $RoleName AND ApplicationId = $ApplicationId";
			...
			try
			{
				using (SQLiteCommand cmd = cn.CreateCommand())
				{
					cmd.CommandText = "SELECT u.Username FROM " + USER_TB_NAME + " u INNER JOIN " + USERS_IN_ROLES_TB_NAME
						...
						+ " WHERE (r.RoleName = $RoleName) AND (r.ApplicationId = $ApplicationId)";
			...
			try
			{
				...
				using (SQLiteCommand cmd = cn.CreateCommand())
				{
					cmd.CommandText = "DELETE FROM " + USERS_IN_ROLES_TB_NAME
						...
						+ " AND RoleId = (SELECT RoleId FROM " + ROLE_TB_NAME + " WHERE RoleName = $RoleName AND ApplicationId = $ApplicationId)";
			...
			try
			{
				using (SQLiteCommand cmd = cn.CreateCommand())
				{
					cmd.CommandText = "SELECT u.Username FROM " + USERS_IN_ROLES_TB_NAME + " uir INNER JOIN " + USER_TB_NAME
						...
						+ " WHERE u.LoweredUsername LIKE $UsernameSearch AND r.RoleName = $RoleName AND u.ApplicationId = $ApplicationId"
