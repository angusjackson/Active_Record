...
namespace TechInfoSystems.Data.SQLite
{
	...
	public sealed class SQLiteMembershipProvider : MembershipProvider
	{
		...
		private const int MAX_APPLICATION_DESCRIPTION_LENGTH = 256;
		private const int MAX_USERNAME_LENGTH = 256;
		private const int MAX_PASSWORD_LENGTH = 128;
		private const int MAX_PASSWORD_SALT_LENGTH = 128;
		...
		private const int MAX_COMMENT_LENGTH = 3000;
		...
		public override void Initialize(string name, NameValueCollection config)
		{
			...
			string appName = GetConfigValue(config["applicationName"], "Gallery Server Pro");

			if (appName.Length > MAX_APPLICATION_NAME_LENGTH)
				throw new ProviderException(String.Format("SQLiteMembershipProvider configuration error: applicationName must be less than or equal to {0} characters. Check the web configuration file (web.config).", MAX_APPLICATION_NAME_LENGTH));
			...
			if (u == null)
			{
				...
				try
				{
					using (SQLiteCommand cmd = cn.CreateCommand())
					{
						...
						cmd.Parameters.AddWithValue("$LastLoginDate", nullDate);
						cmd.Parameters.AddWithValue("$LastPasswordChangedDate", nullDate);
		...
		/// <param name="status">The return status of the method. Possible values are: 0 = User is found & not locked; 
